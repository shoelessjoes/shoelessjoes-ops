> **ARCHIVED 2026-06-16.** Early design spec. The normalized-table model below is largely superseded by
> what's actually built in `packages/core` + Postgres (`ingest-offers`, `poll-messages`, `sync-offers`).
> Kept as a reference for the intended raw→normalized schema. For live behavior see `../AGENT_HANDOFF.md`
> and `../RUNBOOK.md`.
